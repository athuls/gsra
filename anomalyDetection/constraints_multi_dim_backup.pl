...
#data is supposed to hold the values in a dimension for a symbol
our @data;
...
for($i=1;$i<=$numSymbols;$i++)
{
	...
	#For every symbol, create references (=number of dimensions), which refer to arrays to hold values 
	$data[$i-1]=\@arrDimensions;
	for($j=0;$j<$numDimensions;$j++){
		#tempArr to store values
		$ref=\@arrValues;
		$data[$i-1][$j]=$ref;
	}
}	
