...
#
# Small fixes and extensions by:
#  Gwendolyn van der Linden
#  (C) Copyright 2010 SC Solutions, Inc.
#
# $Id: yeanpypa.py,v 1.1 2010/05/21 15:48:02 glinden Exp $
import logging
...
class Literal(Rule):
    ...
    def match(self, input_reader):
        ...
        pos = input_reader.getPos() + 1
        return self.returnToken(self.callAction([self.__string], pos))
...
class AnyOf(Rule):
    ...
    def match(self, input_reader):
        ...
        pos = input_reader.getPos() + 1
        return self.returnToken(self.callAction([char], pos))
...
class AndRule(Rule):
    ...
    def match(self, input_reader):
        ...
        try:
            ...
            raise
        pos = input_reader.getPos() + 1
        return self.returnToken(self.callAction(retval, pos))
    ...
    def match(self, input_reader):
        ...
        for rule in self.__subrules:
            try:
                ...
                input_reader.deleteCheckpoint()
                pos = input_reader.getPos() + 1
                return self.returnToken(self.callAction(rule_match, pos))
            except ParseException:
                pass
        input_reader.rollback()
...
class IgnoreWhite(Rule):
    ...
    def match(self, input_reader):
        ...
        input_reader.setIgnoreState(ignore)
        pos = input_reader.getPos() + 1
        return self.returnToken(self.callAction(result, pos))
...
def parse(parser, string, ignore_white=True):
    ...
    if ignore_white:
        # GWvdL: Make sure remaining whitespace is consumed, to
        # correctly detect fully parsing the input.
        input_reader.skipWhite()
